...
| 历史压力记录容易被当作当前行为证明 | 标注历史记录；更新当前期望并增加 47–52 场景，区分静态覆盖与模型实测 |
...
初始本地检出的四项检查全部通过。发布合并后运行布局、压力场景静态覆盖、运行时 hook、执行前 hook、ISA 查询、ISA 数据质量与统计报告回归；另用官方 `skill-creator/scripts/quick_validate.py` 校验全部 35 个技能。缺少的测试依赖安装到仓库外临时 venv，没有修改项目依赖或宿主插件缓存。

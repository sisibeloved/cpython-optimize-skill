...
- **架构图（框图）**：是设计的第一步，先于其它视图产出。它决定了有哪些设计元素（系统、组件、模块），后续所有视图和模型都必须能在其中找到对应，不得游离其外。
  - 系统内部：体现组件对模块的**包含关系**、模块与模块的**依赖关系**（位置约定：上层框依赖下层框，被依赖者画在下方）。
  - 系统外部：体现与外部系统的**边界和依赖**。
- **上下文视图（框图）**：在架构图基础上，标出本系统与外部系统之间的**一组逻辑接口**（按职责聚合，而非逐个罗列）。
- **流程图**：图中的元素与架构图**一一对应**；两个元素之间的连线代表它们之间的**单个逻辑接口**（函数调用、库调用、RESTful API 等都可表示为一条逻辑接口）。

三类图共享同一套设计元素，且逻辑接口存在聚合关系：上下文图 / 架构图中的一组接口，在流程图里会拆成若干个单接口。例：组件 A 与组件 B 在上下文图中交互的接口是 `IF-DEV-CTRL-API`（按"设备控制"职责聚合成的一组接口，含设备的 CRUD）；而在"创建设备"流程图中，A 调用的是 B 的"创建设备"这一单接口。先有聚合的边界（上下文图 / 架构图），再有细分的调用（流程图），两者的数量和职责必须能对得上。
...
- 流程图与（展开后的）架构图元素**一一对应**，连线代表模块间的单个逻辑接口；上下文图 / 架构图中聚合的一组接口（如 `IF-DEV-CTRL-API`），在具体功能流程图中拆成创建 / 读取 / 更新 / 删除设备等单接口调用。先定边界，再画流程。

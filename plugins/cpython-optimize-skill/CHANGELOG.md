...
- 参考 OpenAI《Rethinking skills and prompts for GPT-6 Astra》收紧全部 35 个技能的触发描述，总字符数由 4,444 降到 2,097，减少约 53%；该统计不代表模型 token 或性能实测。
- 单项任务直接进入对应技能，多阶段 Workflow 按当前证据选择阶段；角色文档按需加载，支持主 Agent 顺序承担职责。
- 沿用用户已有授权与成本预算，减少构建、安装、相关测试、worktree 隔离和远端诊断的重复确认；保留破坏性操作、共享资源和额外成本边界。
- 按受影响行为选择验证范围，复用有效环境证据；提交或报告不再默认触发全量测试。
- 将设计文档入口压缩为模板路由，将 RFC、架构/功能设计的视图规范移入按需引用，保留既有图与模型链约束。
- 执行前 hook 对定向 suite 和单 worker 使用证据提醒，结果比较只路由分析技能；全量、范围不明 helper 和环境改写保留前置检查，ACK 不替代授权或事实证据。

### Fixed

- 修正复合命令中首个低成本动作可能掩盖后续全量运行或环境改写的问题，对已解析的命令片段优先保留 deny。
- 为 ISA 查询服务声明 `mcp>=1,<2` 运行依赖，避免新环境默认安装 MCP 2.x 后缺少 `mcp.server.fastmcp`。

### Added

- 新增仓库维护 `AGENTS.md`、按需角色索引、审计记录与行为场景 49–54；保留 v1.1.0 的 ISA 场景 47–48。
- 扩充执行前 hook 回归，覆盖定向命令、worker 证据、只读结果路由、前置检查、ACK 和复合命令。
